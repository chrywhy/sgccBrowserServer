...
public class UserController_debug {
	static Logger logger = LogManager.getLogger(UserController_debug.class.getName());
    ...
    public User addUser(@PathVariable String uid, @RequestBody @Valid User user) {
    	SqlSession session = DbConnection.getSession().openSession();
		try {
			if (logger.isInfoEnabled()) {
				String passwd = user.getPassword();
